...
layui.use(['form','layer','table','laytpl','util'],function(){
    ...
    var tableIns = table.render({
        elem: '#UserList',
        url : 'http://localhost:8080/xuanke/admin/user/getList?rid='+$("#rid").val(),
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limit : 20,
    ...
    function add(){
        var index = layui.layer.open({
            title : "添加页面",
            type : 2,
            content : "http://localhost:8080/xuanke/admin/user/add?rid="+$('#rid').val(),
    ...
    function edit(data){
        var index = layui.layer.open({
            title : "修改页面",
            type : 2,
            content : "http://localhost:8080/xuanke/admin/user/edit?id="+data.id,
            success : function(layero, index){
    ...
    table.on('tool(UserList)', function(obj){
        ...
        if (Event === 'del') {
            layer.confirm('您确定要删除这条信息？',{icon:3,title:'提示信息'},function(index){
                $.get("http://localhost:8080/xuanke/admin/user/del",{
                    id : Data.id,
                },function(data){
                    if (data.status!=200) {
                        layer.msg(data.info);
