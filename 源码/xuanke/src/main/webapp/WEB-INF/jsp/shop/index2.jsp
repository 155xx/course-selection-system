...
layui.use(['form','layer','table','laytpl','util'],function(){
    ...
    var tableIns = table.render({
        elem: '#List',
        url : 'http://localhost:8080/xuanke/admin/shop/getList2',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limit : 20,
        ...
        cols : [[
            ...
            {field: 'name', title: '课程', minWidth:100, align:'center'},
            {field: 'uname', title: '老师', minWidth:100, align:'center'}
        ]]
    });
    ...
    function add(){
        var index = layui.layer.open({
            title : "添加页面",
            type : 2,
            content : "http://localhost:8080/xuanke/admin/shop/add",
    ...
    function edit(data){
        var index = layui.layer.open({
            title : "修改页面",
            type : 2,
            content : "http://localhost:8080/xuanke/admin/shop/edit?id="+data.id,
            success : function(layero, index){
    ...
    table.on('tool(List)', function(obj){
        ...
        if (Event === 'del') {
            layer.confirm('您确定要删除这条信息？',{icon:3,title:'提示信息'},function(index){
                $.get("http://localhost:8080/xuanke/admin/shop/del",{
                    id : Data.id,
                },function(data){
                    if (data.status!=200) {
                        layer.msg(data.info);
