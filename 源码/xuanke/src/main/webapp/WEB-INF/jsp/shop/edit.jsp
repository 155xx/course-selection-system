...
<form id="mainfrom" class="layui-form" style="width:80%;">
      ...
      <div class="layui-input-block" style="display: flex; align-items: center;">
        <select id="uid" name="uid">
	        <c:forEach var="user" items="${userlist}">
		    	<option value="${user.id}"  <c:if test="${data.uid == user.id}"> selected ="true"</c:if>>${user.name}</option>
	        </c:forEach>
        </select>
      </div>
    </div>
  ...
  <div class="layui-form-item layui-row layui-col-xs12">
    ...
    <div class="layui-input-block">
      <input type="text" name="name" class="layui-input width400 name" value="${data.name}" lay-verify="required" placeholder="请输入课程">
    </div>
  </div>
    ...
    var form = layui.form
        ...
        form.on("submit(edit)",function(data){
            ...
            var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
            	$.ajax({
                      url:'http://localhost:8080/xuanke/admin/shop/edit',
                      type:'post',           //post提交
